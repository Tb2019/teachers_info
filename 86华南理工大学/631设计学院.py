...
partition_num = '020'
start_urls = [
                'https://www2.scut.edu.cn/design/szdw_38586/list.htm',
              ]

a_s_xpath_str = ''
target_div_xpath_str = '//div[@id="container-1" or @id="container-1"]'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        url = "https://faculty.scut.edu.cn/_web/_platform/_teacherHome/api/deptTeacherList.rst?_p=YXM9Mg==&callback=jQuery36005311434427057684_1736993376343&siteId=13&pageIndex=1&rows=100&career=&_=1736993376344"
        ...
        headers = {
            ...
            'cookie': 'Hm_lvt_cad45348d1fdf49a7a9a1f8b99526616=1718596021,1718697859,1718776434,1718861202; JSESSIONID=31AB2B6ECB78558BCDDA2227ABF7AE70',
            'dnt': '1',
            'pragma': 'no-cache',
            'referer': 'https://www2.scut.edu.cn/design/szdw_38586/list.htm',
        ...
        response = json.loads(re.search(r'jQuery.*?\((.*)\)$', response, re.S).group(1))['data']
        for item in response:
            name = item['title']
            link = item['cnUrl']
            phone = item['phone'] if item['phone'] else None
            img = item['headerPic'] if item['headerPic'] else None
            email = item['email'] if item['email'] else None
            yield name, link, phone, img, email
    ...
    def get_detail_page(self, index_result):
        ...
        tasks = [get_response_async(url, session, name=name, phone=phone, img=img, email=email) for name, url, phone, img, email in index_result if len(name) >= 2]

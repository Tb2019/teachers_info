...
partition_num = '020'
start_urls = [
                'https://teacher-sys.app.cetids.com/tmp/2/50/team.js'
              ]

a_s_xpath_str = ''
target_div_xpath_str = '//div[@class="teacher_info"]'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        page = json.loads(re.search(r'=(?=\[)(.*)', index_page).group(1))
        infos = page[0]['children']
        for info in infos:
            teachers = info['teachers']
            for teacher in teachers:
                name = teacher['fullname']
                link_name = teacher['departmentName']
                madhash = teacher['madHash']
                link = f'https://www2.scut.edu.cn/materials/szdw/materials/00/00/c22688a0/page.psp?id={madhash}&value={link_name}'
                yield name, link

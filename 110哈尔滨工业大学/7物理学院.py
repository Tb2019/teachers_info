...
partition_num = '0451'
start_urls = [
                'http://physics.hit.edu.cn/sssds/list.htm',
                'http://physics.hit.edu.cn/sssds/list2.htm',
                'http://physics.hit.edu.cn/sssds/list3.htm',
                'http://physics.hit.edu.cn/sssds/list4.htm',
                'http://physics.hit.edu.cn/sssds/list5.htm',
                'http://physics.hit.edu.cn/sssds/list6.htm',
                'http://physics.hit.edu.cn/sssds/list7.htm',

                'http://physics.hit.edu.cn/bssds/list.htm',
                'http://physics.hit.edu.cn/bssds/list2.htm',
                'http://physics.hit.edu.cn/bssds/list3.htm',
                'http://physics.hit.edu.cn/bssds/list4.htm',
                'http://physics.hit.edu.cn/bssds/list5.htm',

                'http://physics.hit.edu.cn/zrjsdw/list.htm',
                'http://physics.hit.edu.cn/zrjsdw/list2.htm',
                'http://physics.hit.edu.cn/zrjsdw/list3.htm',
                'http://physics.hit.edu.cn/zrjsdw/list4.htm',
                'http://physics.hit.edu.cn/zrjsdw/list5.htm',
                'http://physics.hit.edu.cn/zrjsdw/list6.htm',
                'http://physics.hit.edu.cn/zrjsdw/list7.htm',
              ]

a_s_xpath_str = ' //*[@id="l-container"]/div/div[2]/div/div[2]/div/div/ul/li/a'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            name = a.xpath('.//div[@class="news_title"]/text()')
            if name:
                name = ''.join(name)
                ...
                try:
                    direction = a.xpath('.//div[@class="news_k k2"]/text()')[0].split('：')[1]
                except:
                    direction = None

                try:
                    phone = a.xpath('.//div[@class="news_k k3"]/text()')[0].split('：')[1]
                    if not phone:
                        phone = None
                except:
                    phone = None
                ...
                try:
                    email = a.xpath('.//div[@class="news_k k4"]/text()')[0].split('：')[1]
                    if not email:
                        email = None
                except:
                    email = None
                ...
                continue
            yield name, link, img, direction, phone, email


    # todo：方法二
    # 方法一增加首页获取的信息且不通过自动化工具获取详情页时，需要重写(添加代码)
    '''
    def get_detail_page(self, index_result):
        loop = asyncio.get_event_loop()
        session = aiohttp.ClientSession()
        tasks = [get_response_async(url, session, name=name, img=img, direction=direction, phone=phone, email=email) for name, url, img, direction, phone, email in index_result if len(name) >= 2]
    ...
    def get_detail_page(self, index_result):
        ...
        for name, url, img, direction, phone, email in index_result:
            if len(name) >= 2:
                try:
                    driver.get(url)
                    time.sleep(1)
                ...
                detail_pages.append((page, url, [('name', name), ('img', img), ('direction', direction), ('phone', phone), ('email', email)]))
            else:
                continue
        driver.close()
        return detail_pages

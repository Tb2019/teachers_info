...
partition_num = '021'
start_urls = [
                'https://tdli.sjtu.edu.cn/EN/people/all'
              ]

a_s_xpath_str = ''
target_div_xpath_str = '//div[@class="personnel-detail"]'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        ...
        url = "https://tdli.sjtu.edu.cn/portal-api/people/queryPortalPeopleList"

        payload = "{\"peopleType\":\"ALL\"}"
        headers = {
            ...
            'Cookie': 'LANG=EN; SID=c91efcf31a8f4dd99d28a4691841c100!20240929103026; SID=c91efcf31a8f4dd99d28a4691841c100!20240929103026',
            'DNT': '1',
            'Origin': 'https://tdli.sjtu.edu.cn',
            'Pragma': 'no-cache',
            'Referer': 'https://tdli.sjtu.edu.cn/EN/people/all',
        ...
        for resp in response:
            name = re.sub(r'\(.*?\)', '', resp['name'])
            fir_name = resp["firstName"]
            las_name = resp["lastName"]
            if 'Jointly' in fir_name or 'Jointly' in las_name:
                fir_name = re.sub(r'\(.*?\)', '', fir_name)
                fir_name = re.sub(r'\s', '', fir_name)
                las_name = re.sub(r'\(.*?\)', '', las_name)
                las_name = re.sub(r'\s', '', las_name)
            # fir_name = re.sub(r"(?<!\))\s", "", resp["firstName"])
            # fir_name = re.sub(r"(?<=\()\s", "%20", fir_name)
            # las_name = re.sub(r"(?<!\))\s", "", resp["lastName"])
            # las_name = re.sub(r"(?<=\()\s", "%20", las_name)
                link = 'https://tdli.sjtu.edu.cn/EN/people/all/' + f'{resp["id"]}/' + f'{fir_name + las_name}' + '(Jointly Appointed)'
            else:
                fir_name = re.sub(r'\s', '', fir_name)
                las_name = re.sub(r'\s', '', las_name)
                link = 'https://tdli.sjtu.edu.cn/EN/people/all/' + f'{resp["id"]}/' + f'{fir_name + las_name}'
            yield name, link

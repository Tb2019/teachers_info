...
img_url_head = None
partition_num = '021'
start_urls = ['https://soo.sjtu.edu.cn/active/ajax_teacher_list.html']

a_s_xpath_str = ''
target_div_xpath_str = '//div[@class="m-detbox2"]'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self):
        url = "https://soo.sjtu.edu.cn/active/ajax_teacher_list.html"
        ...
        headers = {
            ...
            'Cookie': 'PHPSESSID=mm8v5imtr67q9tik9cf0p6bip1',
            'DNT': '1',
            'Origin': 'https://soo.sjtu.edu.cn',
            'Pragma': 'no-cache',
            'Referer': 'https://soo.sjtu.edu.cn/teacher-list.html',
        ...
        for i in range(1, 13):
            payload = f"page={i}&cat_code=teacher-list&jsfl=%E5%85%A8%E9%83%A8&xkfx=%E5%85%A8%E9%83%A8&zm=All"

            response = requests.request("POST", url, headers=headers, data=payload).text

            response = re.sub('\\\\/', '/', response)
            response = re.sub(r'\\(r|n|t)', '', response)
            response = re.sub(r'\\(?!u)', '', response)

            for a in re.findall('li>(.*?)</li', response, re.S):
                res = re.search('href="(.*?)".*?span>(.*?)</span', a, re.S)
                try:
                    name = res.group(2).encode().decode('unicode_escape')
                    link = res.group(1)
                    yield name, link
                except:
                    continue

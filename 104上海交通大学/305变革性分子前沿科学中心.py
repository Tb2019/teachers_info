...
school_name = '上海交通大学'
college_name = '变革性分子前沿科学中心'
school_id = 104
college_id = 305
img_url_head = None
partition_num = '021'
start_urls = [
                'https://fsctm.sjtu.edu.cn/ryxx1/ktzz.htm',
              ]

a_s_xpath_str = '//ul[@class="teacher-ul"]/li/div[@class="teacher-txt"]/a'
target_div_xpath_str = '//div[@class="right-nr"]'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        ...
        url = "https://fsctm-faculty.sjtu.edu.cn/info/GetList"
        ...
        headers = {
            ...
            'DNT': '1',
            'Origin': 'https://fsctm.sjtu.edu.cn',
            'Pragma': 'no-cache',
            'Referer': 'https://fsctm.sjtu.edu.cn/',
            ...
            'Cookie': '10.119.14.118:80=22632.61300.21071.0000; ASP.NET_SessionId=aqzo41wt2a0k5bctgfv4uuhl; Abp.Localization.CultureName=zh-CN'
        }
        for mark_page in zip([0, 0, 0, 5], [1, 2, 3, 1]):
            payload = f"mark={mark_page[0]}&page={mark_page[1]}"
            response = requests.request("POST", url, headers=headers, data=payload).json()['result']['items']
            for item in response:
                name = item['name']
                peo_id = item['id']
                link = f'https://fsctm.sjtu.edu.cn/info/1011/3174.htm?id={peo_id}'
                yield name, link

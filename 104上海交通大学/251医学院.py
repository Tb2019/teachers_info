...
partition_num = '021'
start_urls = [
                'https://daoshi.shsmu.edu.cn/Pages/IntroductionHome.aspx',
              ]

a_s_xpath_str = '//div[@id="gvTeacher"]//tbody/tr[position() > 2300 and position() <= 2600]//a'
# a_s_xpath_str = '//div[@id="gvTeacher"]//tbody/tr[position() = 1630]//a'
target_div_xpath_str = '//div[@id="rpBar"]'
    ...
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            ...
            if name:
                ...
                try:
                    link = a.xpath('./@href')[0]
                    link = 'https://daoshi.shsmu.edu.cn' + link.split('Website')[1]
                    if link in ('#',):
                        continue
    ...
    def run(self):
        ...
        for url in self.start_urls:
            ...
            try:
                driver.get(url)
                time.sleep(2)
                driver.find_element(By.XPATH, '//li[@id="rlvProfessional_i0"]').click()
                time.sleep(2)
                frame = driver.find_element(By.XPATH, '//iframe[@id="ifrContent"]')

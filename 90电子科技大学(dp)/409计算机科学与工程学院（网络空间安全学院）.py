...
school_name = '电子科技大学'
college_name = '计算机科学与工程学院（网络空间安全学院）'
school_id = 90
college_id = 409
img_url_head = None
partition_num = '028'
start_urls = ['https://www.scse.uestc.edu.cn/js_sz.jsp?urltype=tree.TreeTempUrl&wbtreeid=1081']

a_s_xpath_str = '//div[@class="fls-fruit"]//a'
    ...
    def run(self):
        ...
        for url in self.start_urls:
            for i in range(1, 20):
                ...
                tab.get(url)
                time.sleep(2)
                tab.ele('@name=fromWenGOPAGE').input(i)
                time.sleep(1)
                tab.ele('@name=fromWenFind').click()
                time.sleep(2)
                index_page = tab.html

...
school_name = '东北农业大学'
college_name = '水利与土木工程学院'
school_id = 5
college_id = 36
start_urls = ['https://slxy.neau.edu.cn/szdw/zrjs/js.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js/4.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js/3.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js/2.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js/1.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/fjs.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/fjs/5.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/fjs/4.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/fjs/3.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/fjs/2.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/fjs/1.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js1.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js1/5.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js1/4.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js1/3.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js1/2.htm',
              'https://slxy.neau.edu.cn/szdw/zrjs/js1/1.htm'
              ]

a_s_xpath_str = '//*[@id="line_u6_0"]/parent::td//table/tbody/tr[1]'
target_div_xpath_str = '//form[@name="_newscontent_fromname"]/div'
# # 研究方向
...
class SpecialSpider(ReCrawler):
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            name = a.xpath('./td[2]//text()')
            # loguru.logger.info(name)
            if name:
                name = ''.join(name)
                name = re.sub(r'\s*', '', name)
                name = re.sub(self.name_filter_re, '', name)
                link = a.xpath('./td[1]/a/@href')[0]
                link = parse.urljoin(url, link)
                img_src = a.xpath('./td[1]/a/img/@src')[0]
                img_src = parse.urljoin(url, img_src)
            else:
                print('未解析到name，请检查：a_s_xpath_str')
                continue
            yield name, link, img_src

    def get_detail_page(self, index_result):
        loop = asyncio.get_event_loop()
        session = aiohttp.ClientSession()
        tasks = [get_response_async(url, session, name=name, img_src=img_src) for name, url, img_src in index_result if len(name) >= 2]

...
college_id = 134
start_urls = [
                'https://basicmed.fudan.edu.cn/azc/list.htm'
              ]

a_s_xpath_str = '//div[@id="wp_news_w8"]/ul/li/div//a[@href!="#"]'
target_div_xpath_str = '//div[@class="shi-teach"]'
        ...
        # if not self.api:  # 不使用第三方api解析
            ...
            if self.phone_xpath:
                ...
                try:
                    ...
                    phone = re.sub(r'(\d{3,4}(?:-|——)\d{4})-(\d{4})', r'\1\2', phone)
                except:
                    pass
            # phone = re.sub(r'——', '-', phone)
...
spider = SpecialSpider(
                   ...
                   phone_pattern=re.compile(r'(?<!\d)1\d{10}(?!\d)|(?<!\d)\d{3,4}(?:-|——)\d{4}-??\d{4}(?!\d)', re.S),

...
college_id = 132
start_urls = [
                'https://mse.fudan.edu.cn/23091/list.htm'
              ]

a_s_xpath_str = '//div[@id="wp_news_w7"]/ul/li//li//a[@href!="#"]'
        ...
        # if not self.api:  # 不使用第三方api解析
            ...
            if self.phone_xpath:
                ...
                try:
                    ...
                    phone = re.sub(r'(\d{8})-\d{3}', r'021-\1', phone)
                except:
                    pass
            # phone = re.sub(r'——', '-', phone)
...
spider = SpecialSpider(
                   ...
                   phone_pattern=re.compile(r'(?<!\d)1\d{10}(?!\d)|(?<!\d)\d{3,4}(?:-|——)\d{7,8}(?!\d)|\d{8}-\d{3}', re.S),

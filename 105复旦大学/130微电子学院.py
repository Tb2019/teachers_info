...
college_id = 130
start_urls = [
                'https://sme.fudan.edu.cn/apy/list.htm#'
              ]

a_s_xpath_str = '//div[@id="wp_news_w6"]/ul/li/ul//a[contains(@href, "htm")]'
target_div_xpath_str = '//div[@class="newscontent"]'

phone_xpath = './div[@class="wzzd clearfix"]/div[@class="news_wz"]/div[@class="news_con"]/div[2]/div[2]/text()'
office_address_xpath = './div[@class="wzzd clearfix"]/div[@class="news_wz"]/div[@class="news_con"]/div[4]/div[2]/text()'
            ...
            if self.phone_xpath:
                try:
                    ...
                    if phone:
                        ...
                        phone = re.sub(r'\(021\)', '021-', phone)
                        phone = re.sub(r'^\+?86-', '0', phone)
                        if len(phone) == 8:
                            phone = '021-' + phone
                        phone = re.sub(r'-\d{3}\s', ',021-', phone)
                        phone = phone if re.search(r'^021', phone) or len(phone) == 11 else '021-' + phone
                except:
                    print('phone_xpath错误')
            else:
                try:

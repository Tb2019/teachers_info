...
college_id = 127
start_urls = [
                'http://www.it.fudan.edu.cn/Data/List/azc'
              ]

a_s_xpath_str = '//div[@class="lanmu_nr"]//a[@href!="#"]'
target_div_xpath_str = '//div[@class="main-right"]'

img_xpath = './/div[@class="teach-right"]/img[@src]/@src'
...
spider = SpecialSpider(
                   ...
                   phone_pattern=re.compile(r'(?<=电话(?:：|：))1\d{10}(?!\d)|(?<=电话(?:：|：))\d{3,4}(?:-|——)\d{7,8}(?!\d)|(?<=电话(?:：|：))\d{8}(?!\d)', re.S),

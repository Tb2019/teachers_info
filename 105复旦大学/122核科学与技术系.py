...
school_name = '复旦大学'
college_name = '核科学与技术系'
school_id = 105
college_id = 122
start_urls = [
                'https://imp.fudan.edu.cn/qtjzglb/list.htm'
              ]

a_s_xpath_str = '//div[@id="wp_content_w15_0"]/table//tr[position()>1]'
target_div_xpath_str = '//div[@class="conny_nr"]'
    ...
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            ...
            if name:
                ...
                try:
                    job_title = a.xpath('./td[2]//text()')[0]
                except:
                    job_title = None
                try:
                    office_address = ''.join(a.xpath('./td[5]//text()'))
                except:
                    office_address = None

                try:
                    phone = a.xpath('./td[3]//text()')[0]
                    phone = re.sub(r'\s*', '', phone)
                except:
                    phone = None

                try:
                    email = a.xpath('./td[4]//text()')[0]
                except:
                    email = None
            else:
    ...
    def parse_detail(self, detail_page, url):
        ...
        # if not self.api:  # 不使用第三方api解析
            ...
            if self.img_xpath:
                ...
                try:
                    src = target_div.xpath('.//img[@src!=""]/@src')[1]

...
school_name = '复旦大学'
college_name = '计算机科学技术学院'
school_id = 105
college_id = 128
start_urls = [
                'https://cs.fudan.edu.cn/24826/list.htm'
              ]

a_s_xpath_str = '//div[@id="wp_news_w6"]/ul/li/ul//a[contains(@href, "htm")]'
target_div_xpath_str = '//div[@class="tearch-intro-box"]'

img_xpath = './div[1]/@style'
        ...
        # if not self.api:  # 不使用第三方api解析
            ...
            if self.phone_xpath:
                ...
                try:
                    ...
                    phone = re.sub(r'86-21', '021', phone)
                except:
                    pass
            # phone = re.sub(r'——', '-', phone)
            ...
            if self.img_xpath:
                try:
                    ...
                    src_str = ''.join(img_list) if ''.join(img_list) else None
                    if src_str:
                        src = re.search(r'background: url\((.*?)\)', src_str, re.S).group(1)
...
spider = SpecialSpider(
                   ...
                   phone_pattern=re.compile(r'(?<!\d)1\d{10}(?!\d)|(?<!\d)\d{3,4}(?:-|——)\d{7,8}(?!\d)|(?<=电话(?:：|:))(?:\+?86-21-)?\d{8}', re.S),

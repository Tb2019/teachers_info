...
school_name = '复旦大学'
college_name = '环境科学与工程系'
school_id = 105
college_id = 125
start_urls = [
                'https://environment.fudan.edu.cn/26241/list.htm'
              ]

a_s_xpath_str = '//div[@class="shi-box"]/ul/li/a[@href!="#"]'
target_div_xpath_str = '//div[@class="person-top"]'


email_xpath = './div[@class="person-tt fl"]/p[contains(text(), "Email")]/text()'
# # 电话
# phone_xpath = None
        ...
        # if not self.api:  # 不使用第三方api解析
            ...
            if self.email_xpath:
                try:
                    ...
                    email = re.sub('(?:Tel.*?)?Email(?:：|:)\s*', '', email)
...
spider = SpecialSpider(
                   ...
                   phone_pattern=re.compile(r'(?<=Tel(?:：|:))\s*?1\d{10}(?!\d)|(?<=Tel(?:：|:))\s*?\d{3,4}(?:-|——)\d{7,8}(?!\d)', re.S),

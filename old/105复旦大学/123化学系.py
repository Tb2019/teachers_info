...
college_id = 123
start_urls = [
                'https://chemistry.fudan.edu.cn/xsdtr/list.htm',
                'https://chemistry.fudan.edu.cn/wjhx/list.htm',
                'https://chemistry.fudan.edu.cn/yjhx/list.htm',
                'https://chemistry.fudan.edu.cn/wlhx/list.htm',
                'https://chemistry.fudan.edu.cn/fxhx/list.htm',
                'https://chemistry.fudan.edu.cn/hxswx_21884/list.htm',
                'https://chemistry.fudan.edu.cn/gljs/list.htm'
              ]

a_s_xpath_str = '//div[@id="wp_news_w6"]/ul/li/div//li//a[@href!="#"]'
target_div_xpath_str = '//div[@class="team"]'
    ...
    def parse_detail(self, detail_page, url):
        ...
        # if not self.api:  # 不使用第三方api解析
            ...
            if self.phone_xpath:
                ...
                try:
                    phone_list = list(set(self.phone_pattern.findall(page)))
                    phone_list.remove('86-21-31242791')
                    if len(phone_list) > 1:
                    ...
                    phone = re.sub(r'86-0?21', '021', phone)
                except:
                    pass
            # phone = re.sub(r'——', '-', phone)
            ...
            if self.email_xpath:
                ...
                try:
                    email_list = list(set(self.email_pattern.findall(page)))
                    if len(email_list) > 1:
            ...
            if self.job_title_xpath:
                ...
                try:
                    job_title = self.job_title_pattern.findall(page)[0]
                except:
                    pass
            ...
            if self.office_address_xpath:
                ...
                try:
                    office_address = re.search(r'<meta.*?办公地点：(?:&lt;/strong&gt;|</strong>)(.*?)(?:&lt;|<)', page, re.S).group(1)
                except:
                    pass
...
spider = SpecialSpider(
                   ...
                   phone_pattern=re.compile(r'(?<!\d)1\d{10}(?!\d)|(?<!\d)\d{3,4}\s?(?:-|——)\d{7,8}(?!\d)|(?:\+?86\s?-)?\s?0?21\s?-\s?\d{8}', re.S),

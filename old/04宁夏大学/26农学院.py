...
school_id = 4
college_id = 26
start_urls = ['https://nxy.nxu.edu.cn/szdw/jsdw1/nyx.htm',
              'https://nxy.nxu.edu.cn/szdw/jsdw1/nyx/1.htm',
              'https://nxy.nxu.edu.cn/szdw/jsdw1/zwycyzx.htm',
              'https://nxy.nxu.edu.cn/szdw/jsdw1/zwycyzx/1.htm',
              'https://nxy.nxu.edu.cn/szdw/jsdw1/nyzyyhjx.htm',
              'https://nxy.nxu.edu.cn/szdw/jsdw1/nyzyyhjx/1.htm',
              'https://nxy.nxu.edu.cn/szdw/jsdw1/zwbhx.htm',
              'https://nxy.nxu.edu.cn/szdw/jsdw1/zwbhx/1.htm',
              ]

a_s_xpath_str = '//div[@class="TeacherCon"]/ul//a'
    ...
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            ...
            if name:
                ...
                responsibilities = re.search(r'职位(?::|：)(.*?)研究方向', name).group(1)
                name = re.sub(self.name_filter_re, '', name)
                link = a.xpath('./@href')[0]
                link = parse.urljoin(url, link)
            else:
                continue
            yield name, link, responsibilities

    def get_detail_page(self, index_result):
        loop = asyncio.get_event_loop()
        session = aiohttp.ClientSession()
        tasks = [get_response_async(url, session, name=name, responsibilities=responsibilities) for name, url, responsibilities in index_result if len(name) >= 2]

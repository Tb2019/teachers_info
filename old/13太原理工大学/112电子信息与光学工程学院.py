...
school_name = '太原理工大学'
college_name = '电子信息有光学工程学院'
school_id = 13
college_id = 112
start_urls = [
                'https://oec.tyut.edu.cn/szdw/js.htm',
                'https://oec.tyut.edu.cn/szdw/fjs.htm',
                'https://oec.tyut.edu.cn/szdw/fyjy.htm',
                'https://oec.tyut.edu.cn/szdw/js1.htm'
              ]

a_s_xpath_str = '//div[@align="justify"]/table//tr[not(@class="firstRow")]'
...
class SpecialSpider(ReCrawler):
    def parse_index(self, index_page, url):
        ...
        for a in a_s:
            name = a.xpath('./td[1]//text()')
            # print(name)
            if name:
                name = ''.join(name)
                name = re.sub(r'\s*', '', name)
                name = re.sub(self.name_filter_re, '', name)
                link = a.xpath('.//a[@href!="#"]/@href')[0]
                link = parse.urljoin(url, link)
                try:
                    directions = a.xpath('./td[2]//text()')[0]

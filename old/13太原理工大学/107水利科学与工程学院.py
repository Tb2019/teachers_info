...
school_name = '太原理工大学'
college_name = '水利科学与工程学院'
school_id = 13
college_id = 107
start_urls = [
                'http://slxy.tyut.edu.cn/szdw/xnjs.htm',
                'http://slxy.tyut.edu.cn/szdw/xnjs/fjs.htm',
                'http://slxy.tyut.edu.cn/szdw/xnjs/fjs/1.htm',
                'http://slxy.tyut.edu.cn/szdw/xnjs/js1.htm',
              ]

a_s_xpath_str = '//div[@id="u_u7_imgcontent"]/table/tbody/tr/td'
...
class SpecialSpider(ReCrawler):
    def parse_index(self, index_page, url):
        ...
        a_s = page.xpath('//div[@style="display:none" and not(@class)]//a/@href')
        names = json.loads(re.search(r'var u_u7_title =(.*?);', index_page, re.S).group(1))
        imgs = json.loads(re.search(r'var u_u7_picurl =(.*?);', index_page, re.S).group(1))
        for name, link, img in zip(names, a_s, imgs):

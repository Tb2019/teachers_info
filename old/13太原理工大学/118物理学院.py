...
college_id = 118
start_urls = [
                'https://wlxy.tyut.edu.cn/szdw/js.htm',
                'https://wlxy.tyut.edu.cn/szdw/fjs.htm',
                'https://wlxy.tyut.edu.cn/szdw/js1.htm'
              ]

a_s_xpath_str = '//table//tr//a'
...
class SpecialSpider(ReCrawler):
    ...
    def parse_index(self, index_page, url):
        ...
        if not a_s:
            print("a_s为空，检查a_s_xpath_str")
        if url == 'https://wlxy.tyut.edu.cn/szdw/js.htm':
            name = None
            for a in a_s:
                link = a.xpath('./@href')[0]
    ...
    def parse_detail(self, detail_page, url):
        if detail_page:
            ...
            if info_s[0][1]:
                ...
                name = re.search(r'姓名(.*?)职称', all_content, re.S).group(1)
                name = re.sub(r'：', '', name)

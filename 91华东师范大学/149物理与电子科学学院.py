...
a_s_xpath_str = '//ul[@class="pic-list"]/li//a[@href!="#"]'
# a_s_xpath_str = '//ul[@class="pic-list"]/li//a[@href="https://faculty.ecnu.edu.cn/_s41/lk2/main.psp"]'
...
class SpecialSpider(ReCrawler):
    def parse_detail(self, detail_page, url):
        ...
        if detail_page:
            ...
            try:
                target_div = tostring(page_tree.xpath(self.target_div_xpath_str)[0], encoding='utf-8').decode('utf-8')
                try:
                    soup = BeautifulSoup(target_div, 'html.parser')
                    element_0 = soup.find(class_='wrapper main0 tabbox')
                    element_1 = soup.find(frag='窗口31')
                    new_tag = BeautifulSoup("<html><body></body></html>", 'html.parser')
                    new_tag.append(element_0)
                    new_tag.append(element_1)
                    target_div = etree.HTML(str(new_tag))
                except:
